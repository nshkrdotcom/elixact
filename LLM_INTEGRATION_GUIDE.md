...
This guide demonstrates how to use Elixact for AI and Large Language Model (LLM) applications, including structured output validation, DSPy integration, and JSON Schema generation for various LLM providers.
...
Elixact provides comprehensive support for LLM integration through:
...
case Elixact.EnhancedValidator.validate(LLMResponseSchema, llm_response, config: config) do
...
defmodule DynamicLLMValidation do
  ...
  def validate_llm_output(response, fields_config) do
    ...
    Elixact.EnhancedValidator.validate(schema, response, config: config)
  end
end
...
defmodule DSPySignature do
  @moduledoc """
  Elixir implementation of DSPy signature patterns using Elixact schemas.
  """
  ...
  defp create_input_schema(fields, opts) do
    ...
    Elixact.Runtime.create_schema(schema_fields,
  ...
  def validate_input(signature, input_data) do
    ...
    Elixact.Runtime.validate(input_data, signature.input, [config: config])
  end

  def validate_output(signature, output_data) do
    config = Elixact.Config.create(strict: true, coercion: :safe)
    Elixact.Runtime.validate(output_data, signature.output, [config: config])
  end

  def get_input_json_schema(signature) do
    schema = Elixact.Runtime.to_json_schema(signature.input)
    
    # Optimize for LLM understanding
    Elixact.JsonSchema.Resolver.optimize_for_llm(schema,
  ...
  def get_output_json_schema(signature, provider \\ :openai) do
    schema = Elixact.Runtime.to_json_schema(signature.output)
    
    # Optimize for specific provider
    Elixact.JsonSchema.Resolver.enforce_structured_output(schema,
      provider: provider,
...
defmodule DSPyProgram do
  ...
  def new(steps, config \\ []) do
    %__MODULE__{
      steps: steps,
      config: Elixact.Config.create(Keyword.put_new(config, :coercion, :safe))
    }
  end
...
defmodule OpenAIIntegration do
  def create_function_schema(schema_module, opts \\ []) do
    ...
    optimized_schema = Elixact.JsonSchema.Resolver.enforce_structured_output(
      json_schema,
      provider: :openai,
      remove_unsupported: true,
  ...
  def validate_function_call(schema_module, arguments) do
    ...
    Elixact.EnhancedValidator.validate(schema_module, arguments, config: config)
  end
end
...
defmodule GenericLLMIntegration do
  def optimize_schema_for_llm(schema, opts \\ []) do
    ...
    schema
    |> Elixact.JsonSchema.Resolver.enforce_structured_output(provider: provider)
    |> Elixact.JsonSchema.Resolver.optimize_for_llm(
  ...
  defp maybe_flatten(schema, true) do
    Elixact.JsonSchema.Resolver.flatten_schema(schema, max_depth: 3)
  end
...
defmodule PromptSchemaGenerator do
  ...
  def create_adaptive_schema(sample_data, opts \\ []) do
    ...
    Elixact.Runtime.create_schema(inferred_fields,
...
defmodule ChainOfThoughtValidation do
  ...
  def validate_chain_of_thought(llm_response) do
    ...
    case Elixact.EnhancedValidator.validate(ChainOfThought, llm_response, config: config) do
      {:ok, validated_cot} ->
...
defmodule FunctionCallingIntegration do
  ...
  defmodule FunctionRegistry do
    ...
    defp create_function_schema(name, func_def, provider) do
      json_schema = Elixact.JsonSchema.from_schema(func_def.schema)
    ...
    defp execute_function_safely(func_def, arguments) do
      ...
      with {:ok, validated_args} <- Elixact.EnhancedValidator.validate(
             func_def.schema, 
             arguments, 
             config: config
           ),
...
defmodule MultiAgentValidation do
  ...
  def validate_agent_interaction(coordination_data) do
    ...
    case Elixact.EnhancedValidator.validate(AgentCoordination, coordination_data, config: config) do
...
defmodule LLMPipelineOrchestration do
  ...
  def execute_pipeline(pipeline, initial_input) do
    ...
    with {:ok, validated_pipeline} <- Elixact.EnhancedValidator.validate(Pipeline, pipeline, config: config),
  ...
  defp execute_single_stage(stage, input, _pipeline) do
    validation_config = Elixact.Config.create(
  ...
  defp validate_stage_input(stage, input, config) do
    Elixact.EnhancedValidator.validate(stage.input_schema, input, config: config)
  end

  defp validate_stage_output(stage, output, config) do
    Elixact.EnhancedValidator.validate(stage.output_schema, output, config: config)
  end
...
defmodule LLMQualityAssessment do
  ...
  def assess_llm_output(content, context \\ nil, opts \\ []) do
    ...
    config = Elixact.Config.create(coercion: :safe, strict: true)
    Elixact.EnhancedValidator.validate(QualityAssessment, assessment_data, config: config)
  end
...
defmodule LLMErrorHandling do
  ...
  defp validate_with_retry_impl(schema, llm_output, retries_left, errors_history) do
    config = Elixact.Config.create(coercion: :aggressive, strict: false)
    
    case Elixact.EnhancedValidator.validate(schema, llm_output, config: config) do
      {:ok, validated} ->
        {:ok, validated}
...
defmodule LLMPerformanceOptimization do
  ...
  @output_schema Elixact.Runtime.create_schema([
    {:result, :string, [required: true]},
    {:confidence, :float, [gteq: 0.0, lteq: 1.0]}
  ])

  @validation_config Elixact.Config.create(coercion: :safe, strict: true)

  def validate_batch_outputs(llm_outputs) do
    Elixact.EnhancedValidator.validate_many(@output_schema, llm_outputs, config: @validation_config)
  end
  ...
  def get_cached_json_schema(schema_key, schema_module) do
    case :ets.lookup(@json_schema_cache, schema_key) do
      ...
      [] ->
        json_schema = Elixact.JsonSchema.from_schema(schema_module)
        optimized_schema = Elixact.JsonSchema.Resolver.optimize_for_llm(json_schema)
...
This comprehensive guide demonstrates how Elixact provides powerful patterns for LLM integration:
...
Key benefits of using Elixact for LLM integration:
