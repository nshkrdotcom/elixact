...
IO.puts("""
🔗 Elixact JSON Schema Resolver Example
=======================================
...
resolved_schema = Elixact.JsonSchema.Resolver.resolve_references(schema_with_refs)
...
resolved_nested = Elixact.JsonSchema.Resolver.resolve_references(nested_schema)
...
resolved_circular = Elixact.JsonSchema.Resolver.resolve_references(circular_schema, max_depth: 3)
...
# Flatten the schema
flattened_schema = Elixact.JsonSchema.Resolver.flatten_schema(complex_schema,
  max_depth: 5,
...
# Optimize for OpenAI
openai_optimized = Elixact.JsonSchema.Resolver.enforce_structured_output(openai_schema,
  provider: :openai,
...
anthropic_optimized = Elixact.JsonSchema.Resolver.enforce_structured_output(anthropic_schema,
  provider: :anthropic,
  add_required_fields: true
)
...
openai_no_formats = Elixact.JsonSchema.Resolver.enforce_structured_output(format_schema,
  provider: :openai,
  remove_unsupported: true
)
...
anthropic_no_formats = Elixact.JsonSchema.Resolver.enforce_structured_output(format_schema,
  provider: :anthropic,
  remove_unsupported: true
)
...
llm_optimized = Elixact.JsonSchema.Resolver.optimize_for_llm(verbose_schema,
...
# Start with a complex schema from a runtime Elixact schema
runtime_schema = Elixact.Runtime.create_schema([
...
base_json_schema = Elixact.Runtime.to_json_schema(runtime_schema)
...
|> Elixact.JsonSchema.Resolver.flatten_schema()
|> Elixact.JsonSchema.Resolver.enforce_structured_output(provider: :openai)
|> Elixact.JsonSchema.Resolver.optimize_for_llm(remove_descriptions: false, simplify_unions: true)
...
result = Elixact.JsonSchema.Resolver.resolve_references(malformed_schema)
IO.puts("✅ Malformed schema handled gracefully")
IO.puts("   Result is still a map: #{is_map(result)}")

# Test with empty schema
empty_schema = %{}
empty_result = Elixact.JsonSchema.Resolver.resolve_references(empty_schema)
...
{time_resolve_us, _} = :timer.tc(fn ->
  for _ <- 1..100 do
    Elixact.JsonSchema.Resolver.resolve_references(benchmark_schema)
  end
end)
...
{time_flatten_us, _} = :timer.tc(fn ->
  for _ <- 1..100 do
    Elixact.JsonSchema.Resolver.flatten_schema(benchmark_schema)
  end
end)
...
{time_optimize_us, _} = :timer.tc(fn ->
  for _ <- 1..100 do
    Elixact.JsonSchema.Resolver.optimize_for_llm(benchmark_schema)
  end
end)
