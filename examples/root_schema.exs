...
# Demonstrates comprehensive patterns for Root Schema validation in Elixact

defmodule RootSchemaExamples do
  @moduledoc """
  Complete examples for Root Schema functionality in Elixact.
  ...
  - Integration with other Elixact features
  ...
  defmodule ContactListSchema do
    use Elixact.RootSchema, root: {:array, ContactInfoSchema}
  end
  ...
  defp enhanced_validator_integration do
    ...
    case Elixact.TypeAdapter.Instance.validate(adapter, numeric_strings) do
  ...
  defp performance_comparison do
    ...
    {adapter_time, _} = :timer.tc(fn ->
      for _ <- 1..iterations do
        Elixact.TypeAdapter.Instance.validate(adapter, data)
      end
    end)
