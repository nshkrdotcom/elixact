...
IO.puts("""
🔧 Elixact TypeAdapter System Example
=====================================
...
for {type, value, description} <- serialization_tests do
  case Elixact.TypeAdapter.dump(type, value) do
    {:ok, serialized} ->
...
email_adapter = Elixact.TypeAdapter.create(:string)
emails = ["user1@example.com", "user2@example.com", "invalid-email", "user3@example.com"]

case Elixact.TypeAdapter.Instance.validate_many(email_adapter, emails) do
  {:ok, validated_emails} ->
...
for {type_spec, description} <- schema_examples do
  schema = Elixact.TypeAdapter.json_schema(type_spec, title: description)
  IO.puts("✅ #{description}:")
...
case Elixact.TypeAdapter.validate(user_profile_type, complex_profile) do
  {:ok, validated} ->
...
perf_adapter = Elixact.TypeAdapter.create({:map, {:string, :integer}})
...
{time_single_us, _} = :timer.tc(fn ->
  Enum.each(test_data, fn item ->
    Elixact.TypeAdapter.validate({:map, {:string, :integer}}, item)
  end)
end)

# Benchmark batch validation
{time_batch_us, _} = :timer.tc(fn ->
  Elixact.TypeAdapter.Instance.validate_many(perf_adapter, test_data)
end)
