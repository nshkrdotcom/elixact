...
IO.puts("""
🚀 Elixact Runtime Schema Generation Example
===========================================
...
IO.puts("   Fields: #{inspect(Elixact.Runtime.DynamicSchema.field_names(user_schema))}")
IO.puts("   Required: #{inspect(Elixact.Runtime.DynamicSchema.required_fields(user_schema))}")
IO.puts("   Optional: #{inspect(Elixact.Runtime.DynamicSchema.optional_fields(user_schema))}")
...
case Elixact.Runtime.validate(valid_user, user_schema) do
  {:ok, validated} ->
...
person_schema = Elixact.Runtime.create_schema(person_fields,
...
case Elixact.Runtime.validate(person_data, person_schema) do
  {:ok, validated} ->
...
json_schema = Elixact.Runtime.to_json_schema(user_schema)
...
basic_schema = Elixact.Runtime.create_schema(basic_fields, title: "Basic Schema")
...
extended_schema = Elixact.Runtime.create_schema(extended_fields, title: "Extended Schema")

IO.puts("Basic schema fields: #{inspect(Elixact.Runtime.DynamicSchema.field_names(basic_schema))}")
IO.puts("Extended schema fields: #{inspect(Elixact.Runtime.DynamicSchema.field_names(extended_schema))}")
...
create_user_schema = fn user_type ->
  ...
  Elixact.Runtime.create_schema(base_fields ++ additional_fields,
...
for user_type <- [:admin, :customer, :guest] do
  ...
  IO.puts("#{user_type} schema fields: #{inspect(Elixact.Runtime.DynamicSchema.field_names(schema))}")
end
...
performance_schema = Elixact.Runtime.create_schema([
  {:id, :integer, [required: true]},
...
{time_us, results} = :timer.tc(fn ->
  Enum.map(test_records, fn record ->
    Elixact.Runtime.validate(record, performance_schema)
  end)
end)
