...
defmodule Elixact.Phase3Example do
  @moduledoc """
  ...
  def run_examples do
    ...
    case UserProfileSchema.validate(user_data) do
      ...
      {:error, errors} ->
        ...
        Enum.each(errors, fn error ->
          IO.puts("   - #{Elixact.Error.format(error)}")
        end)
    end
    ...
    user_json_schema = Elixact.JsonSchema.from_schema(UserProfileSchema)
    IO.puts("✅ User Profile JSON Schema generated!")
    IO.puts("   Properties count: #{map_size(user_json_schema["properties"])}")
    IO.puts("   Computed fields detected: #{Elixact.JsonSchema.has_computed_fields?(user_json_schema)}")
    
    computed_info = Elixact.JsonSchema.extract_computed_field_info(user_json_schema)
    ...
    {input_schema, output_schema} = Elixact.JsonSchema.input_output_schemas(UserProfileSchema)
    ...
    case UserProfileSchema.validate(invalid_user_data) do
      ...
      {:error, errors} ->
        ...
        Enum.each(errors, fn error ->
          IO.puts("   - #{error.code}: #{Elixact.Error.format(error)}")
        end)
    end
  ...
  def demo_integration_features do
    ...
    case Elixact.TypeAdapter.validate(type_spec, user_data) do
      {:ok, validated} ->
    ...
    case Elixact.EnhancedValidator.validate(UserProfileSchema, user_data, config: config) do
      {:ok, validated} ->
    ...
    wrapper = Elixact.Wrapper.create_wrapper(:user_profile, {:ref, UserProfileSchema})
    
    case Elixact.Wrapper.validate_and_extract(wrapper, user_data, :user_profile) do
      {:ok, validated} ->
  ...
  def demo_migration_path do
    ...
    original_json = Elixact.JsonSchema.from_schema(OriginalUserSchema)
    enhanced_json = Elixact.JsonSchema.from_schema(EnhancedUserSchema)
