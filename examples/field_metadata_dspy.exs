...
# Mix.install([{:elixact, path: "."}]) # Commented out for use within Mix project
...
defmodule DSPyFieldProcessor do
  ...
  # Helper to extract JSON type from Elixact type
...
case Elixact.Runtime.validate(test_data, runtime_signature) do
    {:ok, _validated} ->
...
qa_json_schema = Elixact.JsonSchema.from_schema(QASignature)
...
for {field_name, field_schema} <- qa_json_schema["properties"] do
  ...
  if field_schema["x-elixact-extra"] do
    IO.puts("    Extra metadata: #{inspect(field_schema["x-elixact-extra"])}")
  end
end

# Use the enhanced resolver for DSPy optimization
dspy_optimized_schema = Elixact.JsonSchema.EnhancedResolver.optimize_for_dspy(
  QASignature,
  signature_mode: true,
...
missing piece for DSPy integration. This example demonstrates that Elixact
...
🚀 CONCLUSION:
Elixact is READY for DSPy integration! The field metadata system provides
