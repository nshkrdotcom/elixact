...
IO.puts("""
🚀 Elixact Enhanced Validator Example
=====================================
...
case Elixact.EnhancedValidator.validate(UserSchema, user_data) do
  {:ok, validated} ->
...
case Elixact.EnhancedValidator.validate(runtime_schema, product_data) do
  {:ok, validated} ->
...
case Elixact.EnhancedValidator.validate(type_spec, type_data) do
  {:ok, validated} ->
...
strict_config = Elixact.Config.create(%{
  strict: true,
  extra: :forbid,
  coercion: :none,
...
lenient_config = Elixact.Config.create(%{
  strict: false,
  extra: :allow,
  coercion: :safe,
  error_format: :simple
...
case Elixact.EnhancedValidator.validate(UserSchema, test_data, config: lenient_config) do
  {:ok, validated} ->
...
coercion_config = Elixact.Config.create(coercion: :safe)
...
username_type = Elixact.Types.string() 
               |> Elixact.Types.with_constraints([min_length: 2, max_length: 20])

case Elixact.EnhancedValidator.validate_many(username_type, usernames) do
...
case Elixact.EnhancedValidator.validate_with_schema(api_schema, api_request) do
...
llm_schema = Elixact.Runtime.create_schema([
...
debug_schema = Elixact.Runtime.create_schema([
  {:user_id, :integer, [required: true]},
  {:preferences, {:map, {:string, :any}}, [required: true]},
  {:metadata, {:array, {:map, {:string, :string}}}, [required: false]}
])

report = Elixact.EnhancedValidator.validation_report(debug_schema, debug_data)
...
recovery_config = Elixact.Config.create(error_format: :detailed)
...
{time_compiled_us, _} = :timer.tc(fn ->
  Enum.each(benchmark_data, fn data ->
    Elixact.EnhancedValidator.validate(UserSchema, data)
  end)
end)

# Benchmark runtime schema validation
benchmark_schema = Elixact.Runtime.create_schema([
...
{time_runtime_us, _} = :timer.tc(fn ->
  Enum.each(benchmark_data, fn data ->
    Elixact.EnhancedValidator.validate(benchmark_schema, data)
  end)
end)
...
{time_typespec_us, _} = :timer.tc(fn ->
  Enum.each(benchmark_data, fn data ->
    Elixact.EnhancedValidator.validate(type_spec, data)
  end)
end)
...
Enhanced Validator provides a unified interface for all Elixact validation
