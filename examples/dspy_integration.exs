...
IO.puts("""
🔮 Elixact DSPy Integration Pattern Example
==========================================

This example demonstrates how to use Elixact's enhanced features to replicate
...
defmodule DSPyProgram do
  ...
  def validate_output(schema, raw_output, opts \\ []) do
    ...
    Elixact.EnhancedValidator.validate(schema, raw_output, config: config)
  end
...
IO.puts("   Required fields: #{inspect(Elixact.Runtime.DynamicSchema.required_fields(reasoning_schema))}")
...
defmodule QuickValidation do
  ...
  def validate_type(type_spec, value, opts \\ []) do
    Elixact.TypeAdapter.validate(type_spec, value, opts)
  end
  ...
  def validate_llm_primitive(type, value) do
    case type do
      :confidence_score ->
        Elixact.TypeAdapter.validate(:float, value, coerce: true)
        |> case do
      ...
      :string_list ->
        Elixact.TypeAdapter.validate({:array, :string}, value, coerce: true)
      
      :yes_no ->
        case Elixact.TypeAdapter.validate(:string, value, coerce: true) do
          {:ok, str} ->
      ...
      other ->
        Elixact.TypeAdapter.validate(other, value, coerce: true)
    end
  end
end
...
defmodule LLMCoercion do
  ...
  def coerce_llm_output(field_name, type_spec, raw_value, constraints \\ []) do
    Elixact.Wrapper.wrap_and_validate(field_name, type_spec, raw_value,
      coerce: true,
  ...
  def coerce_response_fields(field_specs, raw_response) do
    ...
    case Elixact.Wrapper.validate_multiple(wrappers, raw_response) do
...
defmodule LLMConfigs do
  ...
  def json_mode do
    Elixact.Config.preset(:json_schema)
  end
end
...
simple_schema = Elixact.Runtime.create_schema([
...
for {program_name, input, expected_outputs} <- program_scenarios do
  ...
  case DSPyProgramSimulation.run_program(program_name, input, expected_outputs) do
    ...
    {:error, error_result} ->
      ...
      IO.puts("   Errors:")
      Enum.each(error_result.errors, &IO.puts("     - #{Elixact.Error.format(&1)}"))
  end
end
...
defmodule DSPyRetryPattern do
  ...
  def validate_with_retry(schema, raw_output, max_attempts \\ 3) do
    configs = [
      ...
      Elixact.Config.create(%{strict: true, extra: :forbid, coercion: :none}),
      # Attempt 2: Allow coercion
      Elixact.Config.create(%{strict: true, extra: :forbid, coercion: :safe}),
      # Attempt 3: Lenient validation
      Elixact.Config.create(%{strict: false, extra: :allow, coercion: :aggressive})
    ]
...
{time_enhanced_us, _} = :timer.tc(fn ->
  ...
  Enum.each(test_data, fn data ->
    Elixact.EnhancedValidator.validate(performance_schema, data, config: config)
  end)
end)

{time_runtime_us, _} = :timer.tc(fn ->
  Enum.each(test_data, fn data ->
    Elixact.Runtime.validate(data, performance_schema)
  end)
end)

{time_batch_us, _} = :timer.tc(fn ->
  Elixact.EnhancedValidator.validate_many(performance_schema, test_data)
end)
...
Elixact now provides complete feature parity with Pydantic for DSPy use cases!
""")
