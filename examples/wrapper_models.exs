...
IO.puts("""
🎁 Elixact Wrapper Models Example
=================================
...
score_wrapper = Elixact.Wrapper.create_wrapper(:score, :integer,
...
for {field, type, value, opts} <- one_step_tests do
  case Elixact.Wrapper.wrap_and_validate(field, type, value, opts) do
    {:ok, validated} ->
...
age_wrapper = Elixact.Wrapper.create_flexible_wrapper(:age, :integer, coerce: true)
...
user_wrappers = Elixact.Wrapper.create_multiple_wrappers(user_specs)
...
case Elixact.Wrapper.validate_multiple(user_wrappers, registration_data) do
  {:ok, validated_user} ->
...
url_factory = Elixact.Wrapper.create_wrapper_factory(:string,
...
for {wrapper, value, field} <- factory_tests do
  case Elixact.Wrapper.validate_and_extract(wrapper, value, field) do
    {:ok, validated} ->
...
tags_wrapper = Elixact.Wrapper.create_wrapper(:tags, {:array, :string},
...
metadata_wrapper = Elixact.Wrapper.create_wrapper(:metadata, {:map, {:string, :any}},
...
json_schema = Elixact.Wrapper.to_json_schema(score_wrapper)
...
wrapper_info = Elixact.Wrapper.wrapper_info(score_wrapper)
IO.puts("✅ Score wrapper information:")
IO.inspect(wrapper_info, pretty: true)

# Check if a schema is a wrapper
IO.puts("Is score_wrapper a wrapper? #{Elixact.Wrapper.wrapper_schema?(score_wrapper)}")

# Example 9: Error Handling and Recovery
IO.puts("\n🚨 Example 9: Error Handling and Recovery")

# Create a wrapper with multiple constraints
strict_password_wrapper = Elixact.Wrapper.create_wrapper(:password, :string,
  constraints: [
    min_length: 8,
    max_length: 100,
...
id_wrapper = Elixact.Wrapper.create_wrapper(:id, :integer,
  constraints: [gt: 0],
...
{time_us, results} = :timer.tc(fn ->
  Enum.map(test_ids, fn id ->
    Elixact.Wrapper.validate_and_extract(id_wrapper, id, :id)
  end)
end)
...
{time_one_step_us, _} = :timer.tc(fn ->
  Enum.map(test_ids, fn id ->
    Elixact.Wrapper.wrap_and_validate(:id, :integer, id, constraints: [gt: 0])
  end)
end)
