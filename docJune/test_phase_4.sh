...
check_backward_compatibility() {
    ...
    if mix test test/elixact --exclude integration; then
...
main() {
    ...
    if ! run_test_suite "Phase 4 Core Tests" "test/elixact/phase_4_anonymous_functions_test.exs"; then
        exit_code=1
    fi
    echo ""
    
    # Step 4: Run integration tests
    if ! run_test_suite "Phase 4 Integration Tests" "test/elixact/phase_4_integration_test.exs"; then
        exit_code=1
    fi
    echo ""
...
case "${1:-}" in
    ...
    --format-only)
        ...
        ;;
    --tests-only)
        run_test_suite "Phase 4 Core Tests" "test/elixact/phase_4_anonymous_functions_test.exs"
        core_exit=$?
        run_test_suite "Phase 4 Integration Tests" "test/elixact/phase_4_integration_test.exs"
        integration_exit=$?
