# Elixact Enhanced Features Implementation

## Design Overview

The three missing features will be implemented in a layered approach:

1. **Struct Pattern**: Modify `elixact.ex` to optionally generate structs
...
defmodule ElixactEnhancedFeaturesTest do
  use ExUnit.Case
...
### 1. Enhanced Schema DSL (elixact/schema.ex additions)

```elixir
# Add to elixact/schema.ex
...
### 2. Enhanced Main Module (elixact.ex)
...
defmacro __before_compile__(env) do
  ...
  quote do
    ...
    def validate(data) do
      Elixact.EnhancedSchemaValidator.validate_schema(__MODULE__, data)
    end
    ...
    def dump(struct_or_map) do
      Elixact.EnhancedSchemaValidator.dump(__MODULE__, struct_or_map)
    end
  end
end
```

### 3. Enhanced Schema Validator (elixact/enhanced_schema_validator.ex)

```elixir
# New file: elixact/enhanced_schema_validator.ex

defmodule Elixact.EnhancedSchemaValidator do
  @moduledoc """
  Enhanced validation that supports struct pattern, model validators, and computed fields.
  """
  
  alias Elixact.{Error, FieldMeta}
  ...
  defp validate_basic_fields(fields, data, config, path) do
    ...
    Elixact.Validator.validate_schema_fields(fields, data, config, path)
  end
...
### 4. Enhanced JSON Schema Generation (elixact/json_schema.ex modifications)

```elixir
# Add to elixact/json_schema.ex
...
### 5. Enhanced Runtime Support (elixact/runtime.ex modifications)

```elixir
# Add to elixact/runtime.ex
...
```elixir
# Add to elixact/enhanced_validator.ex
...
enhanced_schema = Elixact.Runtime.create_enhanced_schema([
...
Elixact.Runtime.validate_enhanced(%{input: "test", output: "result"}, enhanced_schema)
```
